...
In the Node Copying method, it is necessary to keep track of which node is the
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    \Require{$M$, size of modifications array}
    \Function{FieldAtVersion}{field $f$, node $n$}
      \State $v \gets n.f$
      \For {$i \in \left\{0, \ldots, M-1\right\}$}
        \If {$n.\textsc{mods}[i].\textsc{version} > v$}
          \State break \textbf{for} loop
        \ElsIf {$n.\textsc{mods}[i].\textsc{field} = f$}
          \If {$n.\textsc{mods}[i].\textsc{version} = v$}
...
If, for instance, one wanted to calculate the sum of the data fields of all
...
\begin{enumerate}
  ...
  Otherwise, look up the value for its \textsc{data} field using algorithm
  \ref{alg:node-copying-get-field-value} and add it to $s$.

  \item Then set $n$ to the value for its own \textsc{next} field using
...
\section{Comparison}
The two approaches have different asymptotic time and space bounds.
