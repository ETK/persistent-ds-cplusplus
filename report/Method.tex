...
\begin{algorithm}{p}
  ...
  \begin{algorithmic}
    \Procedure{EliminateSuperfluousOps}{Batch $B$}
      \For {each operation $o_i \in B$, last operation first}
        ...
        \If {\textsc{type}($o_i$) $\in \left\{\textsc{insert},\textsc{modify}\right\}$}
          \For {each operation $o_j \in \left\{B|j>i\right\}$}
            \Switch {$\textsc{type}(o_j)$}
              ...
              \Case{\textsc{remove}}
                \If{$\textsc{index}(o_j) = c$}
                  \If{$\textsc{type}(o_i)=\textsc{insert}$}
  ...
  %               \If {$\textsc{index}(o_j)=c \land \textsc{type}(o_j) =\textsc{modify}$}
  %                 \State $\textsc{data}(o_i) \gets \textsc{data}(o_j)$
  %                 \State $\textsc{remove}(o_j, B)$
  %               \ElsIf {$\textsc{index}(o_j)=c \land \textsc{type}(o_j) =\textsc{remove}$}
  %                 \If {$\textsc{type}(o_i)=\textsc{insert}$}
  %                   \For {each operation $o_k \in \left\{B|i<k<j\right\}$}
  %                     \If {$\textsc{index}(o_k)>\textsc{index}(o_i)$}
  %                       
  %                     \EndIf
  %                   \EndFor
  %                 \EndIf
  %                 \State \textsc{remove}($o_j,B$)
  %                 \State \textsc{remove}($o_i,B$)
  %                 \State continue with next $o_i$
  %               \Else
  %                 \If{$\textsc{type}(o_j) = \textsc{remove} \land c>\textsc{index}(o_j)$}
  %                   
  %                 \EndIf
  %               \EndIf
          \EndSwitch
