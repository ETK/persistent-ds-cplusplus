...
\begin{columns}[t]
  \begin{column}{0.5\textwidth}
    ...
    \newline 100 seeds: \only<1>{Duration per \textsc{access} operation in milliseconds\vphantom{ --- only head node}}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \newline 10 same-seed runs: \only<1>{Duration per \textsc{access} operation in milliseconds\vphantom{ --- only head node}}
    \only<2>{Duration per \textsc{acccess} operation in milliseconds --- only head node}
  \end{column}
\end{columns}
\end{frame}
